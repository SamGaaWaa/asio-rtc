...
int rtp_packet::write_to(void* data, std::size_t len) const noexcept {
  ...
  header->version_cc = static_cast<uint8_t>(
      (version << 6) | (padding << 5) | (extension << 4) | (csrc_count & 0x0F));
  header->marker_pt =
  ...
  if (extension) {
    ...
    ext->profile_specific = asioice::binary::hton<uint16_t>(extension_profile);
