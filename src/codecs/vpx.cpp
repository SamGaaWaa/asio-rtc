...
class Vp8EncoderImpl : public encoder {
  public:
    ...
    encode(const media_frame &frame, bool force_keyframe) override {
        ...
        // Fill frame with data from media_frame
        if (frame.data.size() >=
            static_cast<size_t>(f->width * f->height * 3 / 2)) {
            // Assume YUV420p
            int y_size = f->width * f->height;
            std::memcpy(f->data[0], frame.data.data(), y_size);
            std::memcpy(f->data[1], frame.data.data() + y_size, y_size / 4);
            std::memcpy(f->data[2], frame.data.data() + y_size + y_size / 4,
                        y_size / 4);
        }
  ...
  public:
    ...
    decode(const std::vector<uint8_t> &rtp_payload,
        ...
        while (true) {
            ...
            std::memcpy(mf.data.data() + y_size, f->data[1], y_size / 4);
            std::memcpy(mf.data.data() + y_size + y_size / 4, f->data[2],
                        y_size / 4);
