...
static void test_parse_video_offer() {
    ...
    auto s = parse_sdp(sdp).value();
    ASSERT(s.version == 0);
    ASSERT(s.groups.size() == 1);
    ASSERT(s.groups[0].items.size() == 1);
    ASSERT(s.groups[0].items[0] == "0");
...
static void test_parse_multiple_media() {
    ...
    auto s = parse_sdp(sdp).value();
    ASSERT(s.groups.size() == 1);
    ASSERT(s.groups[0].items.size() == 3);
    ASSERT(s.groups[0].items[0] == "0");
    ASSERT(s.groups[0].items[1] == "1");
    ASSERT(s.groups[0].items[2] == "2");
...
static void test_roundtrip_session_level_attrs() {
    ...
    ASSERT(parsed.groups[0].items[1] == "1");
...
static void test_unknown_attributes_preserved() {
    ...
    auto str = s.to_string();
    auto reparsed = parse_sdp(str).value();
...
static void test_extmap_parse() {
    ...
    ASSERT(s.medias[0].extmaps[0].uri == "urn:ietf:params:rtp-hdrext:sdes:mid");
