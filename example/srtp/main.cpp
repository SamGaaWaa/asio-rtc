...
static task<void> srtp_session(net::io_context &ctx, ws_ptr ws) {
    ...
    auto srtp_send_task =
        [&](net::io_context &ctx, std::shared_ptr<connection_impl> conn,
        ...
        static const uint8_t vp8_keyframe[] = {
            ...
            0x39, 0xc3, 0xd9, 0x39, 0xc3, 0xd9, 0x39, 0xc3, 0xd9, 0x39, 0xc3,
            0xd9, 0x39, 0xc3, 0xd9, 0x39, 0xc3, 0xd9, 0x39, 0xc3, 0xd9, 0x39,
            0xc3, 0xd9, 0x39, 0x70, 0x00, 0xfe, 0xf8, 0x41, 0xcb, 0xff, 0x7d,
            0x00, 0xff, 0xbd, 0x00, 0xff, 0xbd, 0x00, 0xff, 0x17, 0xfd, 0x77,
            0x8b, 0x49, 0x48, 0x00,
        };
        const size_t vp8_frame_len = sizeof(vp8_keyframe);

        // Build a VP8 RTP packet:
        //   [RTP header 12B] [VP8 payload descriptor (S=1, PID=0)] [VP8 frame
        //   data]
        ...
        frame[12] =
            0x10; // VP8 payload descriptor: S=1 (start of partition), PID=0
        ...
        while (true) {
            ...
            auto [sec, sn] = co_await srtp->send_rtp(orig);
            if (sec) {
                std::cerr << "ICE send_rtp error: " << sec.message() << '\n';
                break;
            }
            send_count++;
            if (send_count % 10 == 0)
