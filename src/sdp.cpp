...
std::pair<std::string_view, std::string_view> split_attr(
...
void apply_session_attr(session_description& session,
...
void apply_media_attr(sdp_media& media,
                      std::string_view name,
                      std::string_view value) {
  std::string vstr(value);
  if (name == "mid") {
    ...
    media.msid = vstr;
    media.attributes.emplace_back("msid", vstr);
