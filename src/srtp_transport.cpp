...
static ::srtp_profile_t to_libsrtp_profile(
    asioice::ssl::srtp_protection_profile p) {
...
static ::srtp_t create_session(const std::vector<uint8_t>& key,
  ...
  if (ret != srtp_err_status_ok)
    throw std::runtime_error{"srtp_create failed: " + std::to_string((int)ret)};
...
srtp_transport_base::~srtp_transport_base() {
  destroy_sessions();
}

std::span<uint8_t> srtp_transport_base::protect_rtp(
    std::span<const uint8_t> input,
    std::span<uint8_t> output) noexcept {
  size_t cap = output.size();
  auto ret = ::srtp_protect(static_cast<::srtp_t>(_send_session), input.data(),
...
std::span<uint8_t> srtp_transport_base::unprotect_rtp(
    std::span<const uint8_t> input,
    std::span<uint8_t> output) noexcept {
  std::size_t cap = output.size();
  auto ret = ::srtp_unprotect(static_cast<::srtp_t>(_recv_session),
...
std::span<uint8_t> srtp_transport_base::protect_rtcp(
    std::span<const uint8_t> input,
    std::span<uint8_t> output) noexcept {
  size_t cap = output.size();
  auto ret =
...
std::span<uint8_t> srtp_transport_base::unprotect_rtcp(
    std::span<const uint8_t> input,
    std::span<uint8_t> output) noexcept {
  std::size_t cap = output.size();
  auto ret =
      ::srtp_unprotect_rtcp(static_cast<::srtp_t>(_recv_session), input.data(),
