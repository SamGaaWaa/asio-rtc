...
struct packet_stream {
    ...
    bool readable() const noexcept { return !_buf.empty(); }

    auto wait_readable() noexcept {
        return asioice::utils::if_else(
            stdexec::just(readable()), [] { return stdexec::just(); },
            [this] {
                return stdexec::just() | stdexec::let_value([this] {
                           return _reader.get_future() |
                                  stdexec::then([this] { return readable(); });
                       }) |
                       exec::repeat_until();
            });
    }

    std::span<const uint8_t> peek() const noexcept { return _buf.peek(); }
    ...
    auto async_write(std::span<const uint8_t> data) {
        ...
        return asioice::utils::if_else(
            stdexec::just(success), [] { return stdexec::just(true); },
            [this, data] {
                return stdexec::just() | stdexec::let_value([this, data] {
                           return _writer.get_future() |
                                  stdexec::then(
                                      [this, data] { return try_write(data); });
                       }) |
                       exec::repeat_until() |
                       stdexec::then([] { return true; }) |
                       stdexec::upon_stopped([] { return false; });
            });
    }
...
using packet_stream_async_write_result =
    std::decay_t<decltype(std::declval<packet_stream &>().async_write(
        std::span<const uint8_t>{}))>;
using packet_stream_wait_readable_result =
    std::decay_t<decltype(std::declval<packet_stream &>().wait_readable())>;
