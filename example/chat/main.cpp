...
asioice::task<void> co_main(net::io_context& ctx) {
  ...
  try {
    ...
    co_await offerer->set_remote_description(parse_sdp(answer_sdp, "answer"));
  } catch (const std::exception& e) {
    std::cout << "Unhandle exception: " << e.what() << '\n';
  }

  while (offerer->ice_connection_state() != ice_connection_state_t::connected &&
         offerer->ice_connection_state() != ice_connection_state_t::failed)
    co_await offerer->on_ice_connection_state_changed();
  while (answerer->ice_connection_state() !=
             ice_connection_state_t::connected &&
         answerer->ice_connection_state() != ice_connection_state_t::failed)
    co_await answerer->on_ice_connection_state_changed();
  if (offerer->ice_connection_state() == ice_connection_state_t::connected &&
      answerer->ice_connection_state() == ice_connection_state_t::connected) {
    std::cout << "Peer connection connected\n";
  } else {
    std::cerr << "Peer connection failed to connecte\n";
    goto END;
  }
END:
