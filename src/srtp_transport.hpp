...
class srtp_transport_base : public asioice::datagram_receiver {
  public:
    ...
    using on_new_ssrc_callback_type = boost::compat::move_only_function<bool(
        uint32_t, std::span<const uint8_t> data)>;
  ...
  private:
    ...
    is_rtcp_packet(std::span<const uint8_t> packet) noexcept {
        if (packet.size() < 8)
            return false;
...
template <class IceTransport> struct srtp_transport : srtp_transport_base {
    ...
    template <VectorLikeBuffer Vec> auto send_rtp(Vec &data) {
    ...
    template <VectorLikeBuffer Vec> auto send_rtcp(Vec &data) {
        std::size_t origin_size = data.size();
