...
std::vector<uint8_t> vpx_payload_descriptor::bytes() const {
    ...
    // 基本头部: I (Extended), P (Partition Start), L (Partition ID 0-7), Part ID (4 bits)
    // Byte 0: 1I 0P L [Part ID]
    // 注意：VP8 RFC 中 I=bit 7, P=bit 4, L=bits 5-6? 
    // 实际上: I (1), P (1), 0 (1), 0 (1), PartID (4) ? 不对。
    // 标准定义: 
...
class Vp8EncoderImpl : public encoder {
  ...
  private:
    void _init_context() {
        ...
        // VP8 实时编码优化参数
        av_opt_set_int(_ctx->priv_data, "cpu-used", 6, 0); // 0-6, 越高越快但质量下降
    ...
    _packetize(const std::vector<uint8_t> &buf) {
        ...
        while (off < buf.size()) {
            ...
            size_t chunk = std::min(buf.size() - off, static_cast<size_t>(PACKET_MAX - dbytes.size()));
...
class Vp8DecoderImpl : public decoder {
  public:
    Vp8DecoderImpl() {
        ...
        if (!codec) throw std::runtime_error{"VP8 Decoder not found"};
        ...
        int ret = avcodec_open2(_ctx, codec, nullptr);
        if (ret < 0) throw std::runtime_error{"avcodec_open2 failed for VP8 decoder"};
    }
